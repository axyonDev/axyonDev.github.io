...
(function(global){
  ...
  let suspended=false,blockingError=null,lastMigration=null,lastSuccessfulSaveAt=0,lastSaveErrorAt=0;
  ...
  function save(state){if(suspended||blockingError){if(blockingError)emit('axyon:save-error',{message:blockingError.message||'Kayıt engellendi',type:blockingError.type||'save'});return false;}try{const id=currentProfileId();if(!id)return false;state.version=global.Axyon.Economy.SAVE_VERSION;state.lastSeen=now();writeState(id,state);touchProfile(id,{gameVersion:global.Axyon.Data.game.version,saveSchema:16,product:'AXYON: Orbital Ascendancy'});lastSuccessfulSaveAt=now();emit('axyon:save-success',{at:lastSuccessfulSaveAt});return true;}catch(e){lastSaveErrorAt=now();blockingError={type:'save',message:e.message,at:lastSaveErrorAt};console.error('[Save v16]',e);emit('axyon:save-error',{message:e.message,type:'save',at:lastSaveErrorAt});return false;}}
  function load(){try{const profile=bootstrap();if(!profile)return null;const id=currentProfileId(),key=saveKey(id),raw=localStorage.getItem(key);if(!raw){const state=global.Axyon.Economy.createInitialState();writeState(id,state);return state;}if(FLAGS.V44_SAVE_V16_ENABLED===false)return normalize(JSON.parse(raw));const ready=ensureV16(key);if(!ready.ok){blockingError={type:'migration',message:ready.error,backupKey:ready.backupKey||''};suspended=true;return global.Axyon.Economy.createInitialState();}const v16=JSON.parse(localStorage.getItem(key)),state=runtimeFromV16(v16);blockingError=null;suspended=false;return state;}catch(e){blockingError={type:'load',message:e.message};suspended=true;console.error('[Load v16]',e);return global.Axyon.Economy.createInitialState();}}
  function resetCurrent(options){const id=currentProfileId();if(!id)return null;const current=blockingError?null:load();suspended=false;blockingError=null;const fresh=global.Axyon.Economy.createInitialState({planetType:options?.planetType||current?.empire?.planetType||current?.planet?.type,startRegion:options?.startRegion||current?.empire?.startRegion||current?.planet?.startRegion});if(options?.theme)fresh.settings.theme=options.theme;writeState(id,fresh);touchProfile(id,{resetAt:now(),saveSchema:16});return fresh;}
  function deleteAll(){suspended=true;try{for(let i=localStorage.length-1;i>=0;i--){const k=localStorage.key(i);if(k&&(k.startsWith(SAVE_PREFIX)||[INDEX_KEY,ACTIVE_KEY,LEGACY_KEY].includes(k)))localStorage.removeItem(k);}blockingError=null;}finally{suspended=false;}}
  ...
  function diagnostics(){return{schema:16,enabled:FLAGS.V44_SAVE_V16_ENABLED!==false,suspended,blockingError,lastMigration,lastSuccessfulSaveAt,lastSaveErrorAt,profile:currentProfile(),canonicalVersion:global.Axyon.Canonical?.version||null,economyEngine:EN?.engine||null,runtimeMode:global.Axyon.Numbers?.runtimeMode||null,firstOrbit:true,product:'AXYON: Orbital Ascendancy'};}
