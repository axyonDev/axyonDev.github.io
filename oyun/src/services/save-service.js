...
(function(global){
  ...
  const now=()=>Date.now(),uid=()=>`cmd_${now().toString(36)}_${Math.random().toString(36).slice(2,9)}`;
  ...
  function save(state){if(suspended||blockingError)return false;try{const id=currentProfileId();if(!id)return false;state.version=global.Axyon.Economy.SAVE_VERSION;state.lastSeen=now();writeState(id,state);touchProfile(id,{gameVersion:global.Axyon.Data.game.version,saveSchema:16});return true;}catch(e){blockingError={type:'save',message:e.message};console.error('[Save v16]',e);return false;}}
