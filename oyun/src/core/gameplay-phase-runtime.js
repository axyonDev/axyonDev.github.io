/**
 * AXYON: Orbital Ascendancy v4.5.6 U4.3.3
 * Factory intelligence, readable requirements and defense-gated ground threats.
 */
(function(global){
  'use strict';
  ...
  const THREAT_MODEL_VERSION=2;
  const GROUND_DEFENSE_TECH='defenseGrid';
  ...
  function groundThreatUnlocked(s){return !!s.researched?.[GROUND_DEFENSE_TECH];}
  ...
  function initializeThreatState(s,t=now(),fresh=false){
    ...
    if(fresh||Number(s.galaxy.threatModelVersion||0)<THREAT_MODEL_VERSION){
      ...
      s.galaxy.groundThreatActivated=!!(s.galaxy.groundThreatActivated||groundThreatUnlocked(s));
      s.galaxy.raidWarningShown=false;
      s.galaxy.nextRaidAt=s.galaxy.spaceDetected?t+D.economyConfig.raidBaseSec*1000:(groundThreatUnlocked(s)&&factoryStarted(s)?t+GROUND_BASE_SEC*1000:0);
      return s;
    }
    s.galaxy.groundThreatLevel=Math.max(1,Math.floor(Number(s.galaxy.groundThreatLevel||1)));
    if(spaceThreatUnlocked(s)&&s.galaxy.threatPhase!=='space'){
      ...
      if(!groundThreatUnlocked(s)){
        s.galaxy.groundThreatActivated=false;s.galaxy.nextRaidAt=0;s.galaxy.raidWarningShown=false;
      }else{
        const newlyActivated=!s.galaxy.groundThreatActivated;
        s.galaxy.groundThreatActivated=true;
        ...
        if(newlyActivated)report(s,'warning','🛡️ Yeryüzü savunma ağı etkin','Gezegen Savunması araştırması tamamlandı. Yerel rakip güçler artık endüstriyel varlığımızı ciddi bir hedef olarak değerlendirebilir.',{category:'intel',phase:'ground',raidAt:s.galaxy.nextRaidAt,technology:GROUND_DEFENSE_TECH});
      }
    }
    return s;
  }
  ...
  function processGroundThreat(s,t){
    if(!groundThreatUnlocked(s))return;
  ...
  function operationFeed(s,t=now()){
    ...
    if(threatPhase(s)==='ground'&&groundThreatUnlocked(s))for(const row of rows)if(row.kind==='raid'){row.title='🚨 Yaklaşan yerel saldırı';row.detail='Yeryüzü güvenliği';}
    return rows;
  }
  ...
  Object.assign(E,{createInitialState,normalizeState,buildRequirements,plantBuildRequirements,canBuild,canBuildPlant,buildMachine,buildPlant,placeMachine,placePlant,removeEntity,tick,tickGalaxyLegacy,applyOfflineProgress,operationFeed,canScan,scanNextTarget,starterFreeRemaining,starterFreePlantRemaining,spaceThreatUnlocked,groundThreatUnlocked,threatPhase,groundSecurityStatus,initializeThreatState});
