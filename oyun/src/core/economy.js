...
(function (global) {
  ...
  function sanitizeState(s,base){
    ...
    s.settings.theme=s.settings.theme==='light'?'light':'dark';s.settings.density=s.settings.density==='compact'?'compact':'wide';
    ...
    const validEntities={};for(const [id,e] of Object.entries(s.grid.entities||{})){if(!e||!['machine','plant'].includes(e.type))continue;const def=e.type==='machine'?mDef(e.defId):pDef(e.defId);if(!def)continue;const x=safeInt(e.x,-1,-1,D.map.size-1),y=safeInt(e.y,-1,-1,D.map.size-1);if(x<0||y<0)continue;validEntities[id]={id,type:e.type,defId:e.defId,x,y};}s.grid.entities=validEntities;s.grid.nextId=safeInt(s.grid.nextId,1,1,MAX_SAFE);s.grid.conveyors=(s.grid.conveyors||[]).filter(x=>x&&((x.from&&x.to&&validEntities[x.from]&&validEntities[x.to])||(Number.isFinite(Number(x.x1))&&Number.isFinite(Number(x.y1))&&Number.isFinite(Number(x.x2))&&Number.isFinite(Number(x.y2))))).slice(0,20000);s.grid.powerLines=(s.grid.powerLines||[]).filter(x=>x&&((x.from&&x.to&&validEntities[x.from]&&validEntities[x.to])||(Number.isFinite(Number(x.x1))&&Number.isFinite(Number(x.y1))&&Number.isFinite(Number(x.x2))&&Number.isFinite(Number(x.y2))))).slice(0,20000);
