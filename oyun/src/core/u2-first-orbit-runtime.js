...
(function(global){
  ...
  function ensureFirstOrbitState(s,options){
    ...
    s.firstOrbit=Object.assign({
      route,landingReactorActive:true,landingReactorPower:Number(D.firstOrbit.starterPackage?.landingReactorPower||120),
  ...
  function computePower(s,dt){let supply=s.firstOrbit.landingReactorActive?Number(s.firstOrbit.landingReactorPower||120):0;for(const d of D.powerPlants){const cnt=s.plants[d.id]?.count||0;if(!cnt)continue;let ratio=1;if(d.fuel){const need=cnt*d.fuel.rate*dt*(1+.12*((s.plantLevels[d.id]||1)-1)),have=toNum(getInv(s,d.fuel.item),Number.MAX_VALUE);ratio=need>0?Math.min(1,have/need):1;setInv(s,d.fuel.item,sub(getInv(s,d.fuel.item),need*ratio));}supply+=Base.plantOutput(s,d.id)*ratio;}let demand=0;for(const d of D.machines)demand+=Base.machinePowerDemand(s,d.id);const ratio=demand?Math.min(1,supply/demand):1;s._power={supply,demand,ratio};return ratio;}
  function runMachine(s,id,seconds,powerRatio){const d=mDef(id),m=s.machines[id];if(!d||!m||m.count<=0){if(m)m.eff=0;return;}let desired=machineRate(s,id)*seconds*powerRatio;if(desired<=0){m.eff=0;return;}let actual=desired;for(const[k,v]of Object.entries(d.recipe.in))actual=Math.min(actual,toNum(div(getInv(s,k),v),Number.MAX_VALUE));for(const[k,v]of Object.entries(d.recipe.out))actual=Math.min(actual,toNum(div(sub(storageCap(s,k),getInv(s,k)),v),Number.MAX_VALUE));actual=Math.max(0,Math.min(desired,actual));m.eff=desired?actual/desired:0;if(!actual)return;for(const[k,v]of Object.entries(d.recipe.in))setInv(s,k,sub(getInv(s,k),actual*v));for(const[k,v]of Object.entries(d.recipe.out)){addInv(s,k,actual*v);s.stats.produced[k]=add(s.stats.produced[k],actual*v);}}
  function manualClick(s,id){const d=mDef(id),m=s.machines[id];if(!d||!m||m.count<1)return dec(0);const out=Object.keys(d.recipe.out)[0],before=getInv(s,out);runMachine(s,id,D.economyConfig.manualBurstSeconds/Math.max(1,m.count),1);return sub(getInv(s,out),before);}
  ...
  function tick(s,dt,t=now()){tickResearch(s,t);tickSectorScans(s,t);tickSatellites(s,t);tickContracts(s,t);const before=Object.fromEntries(itemKeys().map(k=>[k,getInv(s,k)])),power=computePower(s,dt),powerStatus=Object.assign({},s._power);for(const d of sortedMachines){const m=s.machines[d.id];if(m?.count>0&&m.hasManager)runMachine(s,d.id,dt,power);else if(m)m.eff=0;}runMarket(s,t);if(galaxyEventDue(s,t))bridgeGalaxyTick(s,t);s._power=powerStatus;for(const k of itemKeys())s.flow[k]=dt?EN.divSigned(EN.subSigned(getInv(s,k),before[k]),dt):signed(0);s.stats.playTimeSec+=dt;updateTopScore(s);return s;}
  ...
  Object.assign(E,{
    ...
    computePower,tick,manualClick,addCoins,applyOfflineProgress,itemInfo,computeScore,updateTopScore,
