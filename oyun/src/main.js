/** AXYON: Orbital Ascendancy v4.5.6 U4.3.3 — factory intelligence, readable build requirements and defense-gated threats. */
(async function(){
  const E=window.Axyon.Economy,Q=window.Axyon.Quests,S=window.Axyon.SaveService,C=window.Axyon.DomainCommand,Clock=window.Axyon.ServerClock,Reconcile=window.Axyon.ServerReconciliation,Network=window.Axyon.ServerNetwork,UI=window.Axyon.UI,T=window.Axyon.Toast,N=window.Axyon.Numbers,D=window.Axyon.Data,FC=window.Axyon.FactoryCanvas,H=window.Axyon.HelpSystem,CUI=window.Axyon.CombatUI;
  ...
  function refreshCommandStatus(){
    const el=UI.el('command-runtime-status');if(!el||!C)return;const d=commandDiagnostics(),n=Network?.diagnostics?.()||{};
    const net=!n.configured?'Yerel mod':n.status==='online'?'Sunucu çevrimiçi':n.status==='syncing'?'Senkronize ediliyor':n.status==='offline'?'Sunucu çevrimdışı':'Sunucu hazır';
    el.textContent=`Komut revizyonu ${d.revision} · ${d.sources} istemci kaynağı · ${d.pending||0} ACK bekliyor · ${net}`;
    const status=UI.el('authority-network-status');if(status)status.textContent=n.configured?`${net}${n.code?` · ${n.code}`:''}`:'Bağlantı yapılandırılmadı';
  }
  function authorityOptions(){return{actorId:S.currentProfileId(),economy:E,commands:C,reconciliation:Reconcile,save:()=>S.save(state),onChange:()=>{refreshAll();refreshCommandStatus();}};}
  function syncAuthority(){if(!Network?.isConfigured?.()||!S.currentProfileId())return Promise.resolve({status:'disabled'});return Network.syncNow(state,authorityOptions()).then(result=>{refreshCommandStatus();return result;});}
  function executeClientCommand(type,payload,forceServerQueue=false){
    ...
    const queueForServer=forceServerQueue||!!Network?.isConfigured?.(),command=C.create(type,payload||{},{actorId,state,expectedRevision:C.revision(state)}),result=C.execute(state,command,{actorId,economy:E,now:Date.now(),queueForServer});
    if(result.recorded)S.save(state);refreshCommandStatus();if(Network?.isConfigured?.()&&queueForServer&&result.recorded)setTimeout(()=>syncAuthority(),0);return result;
  }
  function runCommand(type,payload){return executeClientCommand(type,payload,false);}
  function queueServerCommand(type,payload){return executeClientCommand(type,payload,true);}
  function applyServerSnapshot(snapshot){
    if(!Reconcile)return{ok:false,code:'reconciliation_runtime_unavailable'};
    const result=Reconcile.apply(state,snapshot,{actorId:S.currentProfileId(),economy:E,commands:C});
    if(result.ok){S.save(state);selectedEntityId=null;refreshAll();refreshCommandStatus();}
    return result;
  }
  ...
  applyDensity(state.settings.density||'wide');
  ...
  const authorityUrl=UI.el('authority-url');if(authorityUrl)authorityUrl.value=Network?.configured?.()||'';refreshCommandStatus();
  const stopAuthoritySync=Network?.startAutoSync?.(()=>state,authorityOptions,5000);

  let interactionBackGuard=false,suppressInteractionPop=false;
  function factoryUiActive(){return FC?.getMode?.()!=='select'||selectedEntityId!==null||!UI.el('fx-inspector')?.classList.contains('hidden')||!UI.el('fx-palette')?.classList.contains('hidden')||!UI.el('fx-status-panel')?.classList.contains('hidden');}
  function armInteractionBackGuard(){
    if(interactionBackGuard||!history?.pushState)return;history.pushState(Object.assign({},history.state||{},{axyonFactoryInteraction:true}),'',location.href);interactionBackGuard=true;
  }
  function releaseInteractionBackGuard(){
    if(!interactionBackGuard||!history?.back)return;interactionBackGuard=false;suppressInteractionPop=true;history.back();
  }
  function cancelFactoryInteraction({fromBack=false,toast=false}={}){
    if(!factoryUiActive())return false;selectedEntityId=null;FC?.cancelInteraction?.();UI.el('fx-palette')?.classList.add('hidden');UI.el('fx-status-panel')?.classList.add('hidden');UI.el('fx-status-toggle')?.setAttribute('aria-expanded','false');UI.showInspector(state,E,null);H?.hide?.();
    if(!fromBack)releaseInteractionBackGuard();if(toast)T.show('Seçim iptal edildi','info');return true;
  }
  function dismissTopUi(fromBack=false){
    const modal=[...document.querySelectorAll('.modal:not(.hidden)')].reverse().find(x=>!x.classList.contains('mandatory-modal'));
    if(modal){UI.hideModal(modal.id);return true;}return cancelFactoryInteraction({fromBack});
  }
  window.addEventListener('popstate',()=>{if(suppressInteractionPop){suppressInteractionPop=false;return;}if(interactionBackGuard){interactionBackGuard=false;dismissTopUi(true);}});
  document.addEventListener('keydown',ev=>{if(ev.key!=='Escape')return;if(dismissTopUi(false)){ev.preventDefault();ev.stopPropagation();}});

  FC.init(UI.el('factory-canvas'),state,E,{
    onModeChange:mode=>{UI.setToolbarMode(mode);if(mode!=='select')armInteractionBackGuard();},
    onSelect:e=>{selectedEntityId=e.id;UI.showInspector(state,E,e);armInteractionBackGuard();},
    onChange:()=>{refreshAll();},
    onPlaceEntity:(defId,kind,x,y)=>{const r=runCommand('factory.place',{defId,kind,x,y});return r.ok?r.data?.entityId:null;},
    onMoveEntity:(entityId,x,y)=>runCommand('factory.move',{entityId,x,y}).ok,
    onRemoveEntity:entityId=>runCommand('factory.remove',{entityId}).ok,
    onAddConveyor:(from,to)=>runCommand('factory.add-conveyor',{from,to}).ok,
    onAddPowerLine:(from,to)=>runCommand('factory.add-power-line',{from,to}).ok,
    onRemoveLine:(x,y,radius)=>runCommand('factory.remove-line',{x,y,radius}).ok,
    ...
    onNodeClick:(node,gx,gy)=>{selectedEntityId=null;UI.showNodeBuilder(state,E,node.type,gx,gy);armInteractionBackGuard();},
  ...
  function showOfflineResult(){/* Kısa dönüşler ve sıradan üretim ilerlemesi sessizdir. Yalnız gerçek olaylar stratejik rapor akışına yazılır. */}
  let lastStrategicReportId=state.galaxy.reports?.[0]?.id||null;
  function surfaceStrategicNews(){const latest=state.galaxy.reports?.[0];if(!latest||latest.id===lastStrategicReportId)return;lastStrategicReportId=latest.id;const cat=latest.details?.category;if(['raid','ground-raid','intel','battle'].includes(cat))T.show(`${latest.title}<br><span class="toast-reward">${latest.body}</span>`,latest.type==='raid-loss'||latest.type==='battle-loss'?'error':latest.type==='warning'?'info':'success');}
  function applyResumeProgress(){if(firstRun||resumeLock)return{usableSeconds:0};resumeLock=true;try{const result=E.applyOfflineProgress(state);last=performance.now();showOfflineResult(result);surfaceStrategicNews();S.save(state);refreshAll();return result;}finally{resumeLock=false;}}
  ...
  function tryOpenSector(sx,sy){const r=runCommand('sector.open',{sx,sy});if(r.ok){T.show(`🧭 Sektör taraması başladı: ${sx},${sy}`,'success');postAction();}else T.show('Komşu sektör, tarama modülü veya malzemeler yetersiz','error');}
  ...
  // Canvas araçları
  document.querySelectorAll('.fx-tool[data-mode]').forEach(b=>b.addEventListener('click',()=>{if(b.dataset.mode==='select')cancelFactoryInteraction();else FC.setMode(b.dataset.mode);}));
  ...
  UI.el('fx-status-toggle').addEventListener('click',()=>{const panel=UI.el('fx-status-panel'),open=panel.classList.contains('hidden');panel.classList.toggle('hidden',!open);UI.el('fx-status-toggle').setAttribute('aria-expanded',String(open));if(open){UI.renderFactoryStatus(state,E);armInteractionBackGuard();}else if(!factoryUiActive())releaseInteractionBackGuard();});
  UI.el('fx-status-close').addEventListener('click',()=>{UI.el('fx-status-panel').classList.add('hidden');UI.el('fx-status-toggle').setAttribute('aria-expanded','false');if(!factoryUiActive())releaseInteractionBackGuard();});
  UI.el('fx-build-toggle').addEventListener('click',()=>{UI.el('fx-palette').classList.toggle('hidden');if(!UI.el('fx-palette').classList.contains('hidden'))armInteractionBackGuard();else if(!factoryUiActive())releaseInteractionBackGuard();});UI.el('fx-palette-close').addEventListener('click',()=>{UI.el('fx-palette').classList.add('hidden');if(!factoryUiActive())releaseInteractionBackGuard();});
  ...
  UI.el('fx-palette').addEventListener('click',ev=>{const b=ev.target.closest('[data-place]');if(!b)return;FC.setMode('place',b.dataset.place,b.dataset.ptype);UI.el('fx-palette').classList.add('hidden');armInteractionBackGuard();});
  UI.el('fx-inspector').addEventListener('click',ev=>{
    if(ev.target.id==='fxi-close'){cancelFactoryInteraction();return;}
    const nodeBuild=ev.target.closest('[data-build-node]');if(nodeBuild){
      const inspector=UI.el('fx-inspector'),x=Number(inspector.dataset.nodeX),y=Number(inspector.dataset.nodeY),id=nodeBuild.dataset.buildNode;
      const r=runCommand('factory.place',{defId:id,kind:'machine',x,y});
      FC.cancelInteraction();UI.el('fx-palette').classList.add('hidden');
      if(r.ok){selectedEntityId=r.data?.entityId||null;const ent=selectedEntityId?state.grid.entities[selectedEntityId]:null;UI.showInspector(state,E,ent||null);T.show(`⛏️ ${E.mDef(id)?.name||id} yatağa kuruldu`,'success');UI.buildPalette(state,E);postAction();}
      else{selectedEntityId=null;UI.showInspector(state,E,null);T.show('Bu yatağa kurulum için teknoloji, kapasite veya kuruluş hakkı yetersiz','error');}
      if(!selectedEntityId)releaseInteractionBackGuard();return;
    }
    const b=ev.target.closest('[data-fxi]'),ent=state.grid.entities[UI.el('fx-inspector').dataset.entity];if(!b||!ent)return;
    if(b.dataset.fxi==='run'){const r=runCommand('machine.manual-run',{machineId:ent.defId}),g=r.data?.produced||0;T.show(r.ok?`▶ +${N.format(g)} üretildi`:'Girdi veya depo yetersiz',r.ok?'success':'info');}
    if(b.dataset.fxi==='automation'){const r=runCommand('machine.upgrade-automation',{machineId:ent.defId});if(r.ok){const lv=E.automationLevel(state,ent.defId);T.show(`🧠 Otomasyon çekirdeği Seviye ${lv} oldu`,'success');}else T.show('Teknoloji, kredi veya otomasyon malzemeleri yetersiz','error');}
    if(b.dataset.fxi==='upgrade'){const r=runCommand('structure.upgrade-class',{structureId:ent.defId,kind:ent.type});if(r.ok)T.show(`⬆ ${ent.type==='plant'?E.pDef(ent.defId).name:E.mDef(ent.defId).name} yükseltildi`,'success');else T.show('Teknoloji, malzeme veya kredi yetersiz','error');}
  ...
  // Güç paneli
  UI.el('plants-container').addEventListener('click',ev=>{const build=ev.target.closest('[data-buildplant]'),up=ev.target.closest('[data-upgradeplant]');if(build){const id=build.dataset.buildplant;T.show('Santrali gezegen yüzeyine yerleştir; ardından ⚡ Hat aracıyla makinelere bağla','info');FC.setMode('place',id,'plant');armInteractionBackGuard();UI.switchTab('factory');}if(up){const r=runCommand('structure.upgrade-class',{structureId:up.dataset.upgradeplant,kind:'plant'});if(r.ok)T.show('⚡ Santral sınıfı yükseltildi','success');else T.show('Teknoloji veya kaynak yetersiz','error');postAction();}});
  ...
  UI.el('inventory-list').addEventListener('click',ev=>{
    ...
    else if(auto)runCommand('market.toggle-item-auto-sell',{itemId:auto.dataset.auto});
    else if(stor){const r=runCommand('storage.upgrade',{itemId:stor.dataset.stor});if(r.ok)T.show('📦 Depo kapasitesi büyüdü','success');else T.show('Kredi yetersiz','error');}
    else if(keep)runCommand('market.set-item-keep',{itemId:keep.dataset.keep,pct:Number(keep.dataset.pct)});
    else return;refreshAll();
  });
  ...
  UI.el('research-active').addEventListener('click',ev=>{if(ev.target.id!=='cancel-research')return;if(!confirm('Aktif araştırma iptal edilsin mi? Harcananların %70’i iade edilir.'))return;const r=runCommand('research.cancel',{});if(r.ok){T.show('Araştırma iptal edildi · %70 iade yapıldı','info');refreshAll();}});
  ...
  function rebuildForState(){firstRun=!S.currentProfile();interactionBackGuard=false;suppressInteractionPop=false;FC.setState(state);FC.cancelInteraction?.();applyTheme(state.settings.theme||'dark');applyDensity(state.settings.density||'wide');UI.buildInventory(state,E);UI.buildResearch(state,E);UI.buildPlantCards(state,E);UI.buildPalette(state,E);selectedEntityId=null;UI.showInspector(state,E,null);refreshAll();}
  ...
  UI.el('authority-connect')?.addEventListener('click',async()=>{const url=Network?.configure?.(UI.el('authority-url')?.value||'');if(!url){T.show('Geçerli bir http/https sunucu adresi girin','error');refreshCommandStatus();return;}T.show('Sunucu bağlantısı deneniyor…','info');const result=await syncAuthority();T.show(result.status==='online'?'🌐 Sunucu senkronizasyonu tamamlandı':`Sunucu bağlantısı kurulamadı: ${result.code||result.status}`,result.status==='online'?'success':'error');refreshCommandStatus();});
  UI.el('authority-disconnect')?.addEventListener('click',()=>{Network?.configure?.('');if(UI.el('authority-url'))UI.el('authority-url').value='';refreshCommandStatus();T.show('Yerel çevrimdışı moda geçildi','info');});
  ...
  document.querySelectorAll('[data-density-set]').forEach(btn=>btn.addEventListener('click',()=>{const d=btn.getAttribute('data-density-set')==='compact'?'compact':'wide';state.settings.density=d;applyDensity(d);S.save(state);}));
  ...
  UI.el('do-reset').addEventListener('click',async()=>{
    if(UI.el('reset-confirm-text').value.trim().toLocaleUpperCase('tr-TR')!=='SIFIRLA')return;
    if(!confirm('SON ONAY: Aktif profil; yerel kayıt ve bağlı yetkili sunucu dahil yeni oyun durumuna döndürülecek. Bu işlem geri alınamaz.'))return;
    const payload={theme:state.settings.theme,planetType:state.empire?.planetType||state.planet?.type,startRegion:state.empire?.startRegion||state.planet?.startRegion};
    let result=null;
    if(S.hasBlockingError?.())state=S.resetCurrent(payload);
    else{result=runCommand('profile.reset',payload);if(!result.ok){T.show(`Sıfırlama uygulanamadı: ${result.code||'bilinmeyen hata'}`,'error');return;}}
    lastStrategicReportId=null;rebuildForState();UI.el('reset-confirm-text').value='';UI.el('do-reset').disabled=true;UI.hideModal('settings-modal');FC.recenter();
    let sync=null;if(Network?.isConfigured?.())sync=await syncAuthority();
    const pending=C?.diagnostics?.(state)?.pending||0;
    if(sync?.status==='offline'||pending)T.show('🧹 Harita yerelde sıfırlandı. Sunucu sıfırlaması bağlantı kurulunca güvenle tamamlanacak.','info');
    else T.show('🧹 Profil sıfırlandı: harita boş; 7 ücretsiz makine, 1 ücretsiz kömür jeneratörü ve 12 kömürlük kuruluş yakıtı hazır','success');
  });
  ...
  window.__axyon={get state(){return state;},E,S,Q,C,Clock,Network,UI,runCommand,queueServerCommand,syncAuthority,applyServerAck,applyServerSnapshot,executeCommandEnvelope:(command)=>{const actorId=S.currentProfileId(),result=C.execute(state,command,{actorId,economy:E,now:Date.now()});if(result.recorded)S.save(state);refreshCommandStatus();return result;}};
