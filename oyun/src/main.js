/** AXYON: Orbital Ascendancy v4.5.5 U4.3.2 — strict power lines, direct resource builds and input cancellation. */
(async function(){
  ...
  function factoryUiActive(){return FC?.getMode?.()!=='select'||selectedEntityId!==null||!UI.el('fx-inspector')?.classList.contains('hidden')||!UI.el('fx-palette')?.classList.contains('hidden');}
  ...
  function cancelFactoryInteraction({fromBack=false,toast=false}={}){
    if(!factoryUiActive())return false;selectedEntityId=null;FC?.cancelInteraction?.();UI.el('fx-palette')?.classList.add('hidden');UI.showInspector(state,E,null);H?.hide?.();
