...
const cell=findCell(s,'ironMine');assert(cell&&E.placeMachine(s,'ironMine',...cell));assert.strictEqual(E.starterFreeRemaining(s,'ironMine'),0);assert.strictEqual(s.galaxy.nextRaidAt,0,'early-game ground threat must remain locked before defense research');
...
// Ground phase: early game is safe; first defense technology activates real local threats.
...
assert.strictEqual(E.groundThreatUnlocked(ground),false);assert.strictEqual(ground.stats.raidsWon+ground.stats.raidsLost,0);assert.strictEqual(ground.galaxy.nextRaidAt,0);
ground.researched.defenseGrid=true;E.initializeThreatState(ground,Date.now());assert(E.groundThreatUnlocked(ground));assert(ground.galaxy.nextRaidAt>Date.now());ground.galaxy.nextRaidAt=Date.now()-1;E.tick(ground,.1,Date.now());
...
console.log('PASS U4.3.1: empty-map reset, manual free starter deployment, defense-gated ground threats and First-Orbit space gate');
