...
(async()=>{
  ...
  const health=await fetch(`${app.base}/health`);const healthBody=await health.json();assert.strictEqual(healthBody.version,'4.5.6-u4.3.3');assert.strictEqual(healthBody.diagnostics.backend,'sqlite');
