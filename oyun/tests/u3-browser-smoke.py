...
def main():
    ...
    with sync_playwright() as pw:
        ...
        # Background resume remains idempotent, but ordinary elapsed time is
        # intentionally silent in the Groundfront strategy identity.
        bg = page.evaluate(
            """
            (()=>{
              const s=window.__axyon.state;
              ...
              const firstLastSeen=s.lastSeen,earned=window.Axyon.EconomyNumber.toStorage(s.totalEarned);
              document.dispatchEvent(new Event('visibilitychange'));
              const toast=document.getElementById('toast-layer').textContent;
              return {modalExists:!!document.getElementById('offline-modal'),firstLastSeen,secondLastSeen:s.lastSeen,earnedAfterSecond:window.Axyon.EconomyNumber.toStorage(s.totalEarned),earned,toast};
            })()
            """
        )
        result["background"] = bg
        if bg["modalExists"] or bg["earned"] != bg["earnedAfterSecond"] or bg["firstLastSeen"] != bg["secondLastSeen"] or "Komutan geri döndü" in bg["toast"] or "çevrimdışı ilerleme" in bg["toast"]:
