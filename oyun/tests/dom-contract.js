...
assert(sw.includes('axyon-orbital-ascendancy-v4-4-1-u3-1'),'service worker cache version is stale');assert(cached.includes('data/u2-first-orbit-data.js')&&cached.includes('src/core/u2-first-orbit-runtime.js'),'U2 runtime is not cached for offline mode');assert(cached.includes('data/u3-planetary-bastions-data.js')&&cached.includes('src/core/u3-planetary-bastions-runtime.js'),'U3 runtime is not cached for offline mode');assert(cached.includes('src/ui/combat-ui.js'),'combat UI is not cached');
const manifest=JSON.parse(read('manifest.json'));assert(manifest.name.includes('Orbital Ascendancy'),'manifest product name is stale');assert((manifest.version||'').includes('4.4.1-u3.1'),'manifest version is stale');
