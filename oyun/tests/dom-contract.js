...
assert(encyclopedia.includes('data/u2-first-orbit-data.js'),'encyclopedia is not connected to U2 data bridge');
...
assert(sw.includes('axyon-first-orbit-v4-4-u2'),'service worker cache version is stale');assert(cached.includes('data/u2-first-orbit-data.js')&&cached.includes('src/core/u2-first-orbit-runtime.js'),'U2 runtime is not cached for offline mode');assert(cached.includes('src/ui/combat-ui.js'),'combat UI is not cached');
const manifest=JSON.parse(read('manifest.json'));assert(manifest.name.includes('First Orbit'),'manifest product name is stale');assert((manifest.version||'').includes('4.4.0-u2'),'manifest version is stale');

const scriptOrder=[...index.matchAll(/<script\s+src=["']([^"']+)["']/g)].map(m=>m[1]);
for(const required of ['src/core/numbers.js','data/config.js','data/u2-first-orbit-data.js','src/core/economy.js','src/core/u2-first-orbit-runtime.js','src/services/save-service.js','src/ui/help-system.js','src/canvas/factory-canvas.js','src/ui/ui.js','src/ui/combat-ui.js','src/main.js'])assert(scriptOrder.includes(required),`missing script ${required}`);
assert(scriptOrder.indexOf('data/config.js')<scriptOrder.indexOf('data/u2-first-orbit-data.js'),'config must load before U2 data bridge');assert(scriptOrder.indexOf('data/u2-first-orbit-data.js')<scriptOrder.indexOf('src/core/economy.js'),'U2 data bridge must load before economy');assert(scriptOrder.indexOf('src/core/economy.js')<scriptOrder.indexOf('src/core/u2-first-orbit-runtime.js'),'legacy core must load before U2 runtime');assert(scriptOrder.indexOf('src/core/u2-first-orbit-runtime.js')<scriptOrder.indexOf('src/services/save-service.js'),'U2 runtime must load before save service');
...
console.log('PASS dom-contract U2: static ids, local assets, offline cache, U2 script order, encyclopedia bridge and responsive First Orbit UI');
