...
assert.strictEqual(Object.keys(fresh.grid.entities).length,0,'new game must start with an empty factory map');assert.strictEqual(E.starterFreeRemaining(fresh,'ironMine'),1);
...
assert.strictEqual(fresh.firstOrbit.landingReactorPower,0);assert.strictEqual(fresh.firstOrbit.landingReactorActive,false);assert.strictEqual(E.starterFreePlantRemaining(fresh,'coalGen'),1);assert(decimalEq(ctx,fresh.inventory.coal,12),'finite landing coal cargo missing');
const freshMachineIds=[];for(const id of Object.keys(D.firstOrbit.starterPackage.machines)){const cell=findCell(fresh,id),eid=cell&&E.placeMachine(fresh,id,...cell);assert(eid,`starter right could not be placed: ${id}`);freshMachineIds.push(eid);assert.strictEqual(E.starterFreeRemaining(fresh,id),0);}
E.tick(fresh,1,Date.now()+1000);assert(EN.eq(fresh.stats.produced.coal,0),'unpowered starter production must remain zero');
const freshPlantCell=findCell(fresh,'coalGen','plant'),freshPlantId=freshPlantCell&&E.placePlant(fresh,'coalGen',...freshPlantCell);assert(freshPlantId,'starter coal generator could not be placed');assert.strictEqual(E.starterFreePlantRemaining(fresh,'coalGen'),0);for(const eid of freshMachineIds)assert(E.addPowerLine(fresh,freshPlantId,eid));
E.tick(fresh,30,Date.now()+30000);
assert(EN.gt(fresh.stats.produced.coal,0),'powered starter production did not run');assert(EN.lt(fresh.inventory.coal,EN.add(12,fresh.stats.produced.coal)),'coal generator consumed no fuel');
...
const compat=E.createInitialState();fillEconomy(ctx,compat,'1e12');D.research.forEach(t=>compat.researched[t.id]=true);const compatCell=findCell(compat,'ironMine');assert(compatCell&&E.placeMachine(compat,'ironMine',...compatCell));
...
compat.market.prototypeBuilt=true;compat.galaxy.satellites.prototypeMarketSatellite=1;E.initializeThreatState(compat,Date.now());compat.galaxy.scanCooldownUntil=0;assert(E.scanNextTarget(compat));assert(compat.coins instanceof EN.Decimal,'galaxy scan downgraded credits');

// Offline production continues, but due punitive raids receive the preparation window.
const offline=E.createInitialState();offline.researched.defenseGrid=true;E.initializeThreatState(offline,Date.now());const offlineCell=findCell(offline,'ironMine'),offlineMine=offlineCell&&E.placeMachine(offline,'ironMine',...offlineCell);assert(offlineMine);const offlinePlantCell=findCell(offline,'coalGen','plant'),offlinePlant=offlinePlantCell&&E.placePlant(offline,'coalGen',...offlinePlantCell);assert(offlinePlant&&E.addPowerLine(offline,offlinePlant,offlineMine));offline.lastSeen=Date.now()-3600_000;offline.galaxy.nextRaidAt=offline.lastSeen+1000;const raidCount=offline.stats.raidsWon+offline.stats.raidsLost;
...
console.log('PASS u2-first-orbit: Decimal runtime, zero-credit manual-deployment industry, discoveries, Mk0 satellite, guaranteed contracts, Market Mk I, satellite-only sales, legacy bridges and offline raid safety');
