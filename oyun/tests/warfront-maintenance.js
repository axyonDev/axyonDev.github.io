...
const r=E.createInitialState();fill(r);for(const d of D.defenses)r.galaxy.defenses[d.id]=100;r.inventory.ammunition=EN.safe('1e9');r.galaxy.threat=3;const entities=Object.keys(r.grid.entities).length;r.galaxy.nextRaidAt=Date.now()-1;E.tick(r,.1,Date.now());const raid=r.galaxy.reports.find(x=>x.details?.category==='raid');assert(raid&&raid.details.phases.length===3);assert.strictEqual(Object.keys(r.grid.entities).length,entities);assert(['planet','orbital','satellite'].some(k=>r.maintenance.integrity[k]<100));

// Procedural frontier continues after fixed targets.
const f=E.createInitialState();fill(f);f.galaxy.targets.forEach(t=>t.discovered=true);f.galaxy.scanCooldownUntil=0;const frontier=E.scanNextTarget(f);assert(frontier&&frontier.procedural&&frontier.frontierDepth===1);f.galaxy.scanCooldownUntil=0;frontier.defeated=true;f.galaxy.frontierGenerated=1;const frontier2=E.scanNextTarget(f);assert(frontier2&&frontier2.frontierDepth===2);assert(frontier2.strength>frontier.strength&&frontier2.distance>frontier.distance);
