...
## v4.5.6 U4.3.3 — Factory Intelligence & Defense-Gated Groundfront

Bu sürüm, canlı aggregate fabrika motorunu oyuncuya daha açık anlatır ve erken oyunu gereksiz saldırı baskısından korur. İnşa gereksinimleri artık **simge + ürün adı + adet** gösterir; Yeryüzü Cephesi ilk gerçek savunma araştırması tamamlanana kadar kapalı kalır; harita üzerinde açılıp kapanabilen fabrika durum çekmecesi her yerleşik yapının üretim, çevrim ve güç durumunu raporlar.

## U4.3.3 öne çıkanlar

- İnşa paletindeki maliyetler yalnız emoji yerine `simge + ürün adı + adet` biçiminde gösterilir.
- Ücretsiz kuruluş hakları ve alan/enerji gereksinimleri açık metinle ayrıştırılır.
- Yerel tehdit kapısı ilk savunma teknolojisi **Gezegen Savunması (`defenseGrid`)** olarak sabitlendi.
- `defenseGrid` araştırılmadan yerel baskın zamanlayıcısı çalışmaz; HUD tehdit değeri `0` kalır.
- Araştırma tamamlandığında Yeryüzü Cephesi zamanlayıcısı başlar ve tek bir istihbarat raporu üretilir.
- Fabrika haritasına açılır `📈 Durum` çekmecesi eklendi.
- Çekmece; aktif/toplam makine, arz/talep/teslim edilen güç ve bağlantısız yapı sayılarını gösterir.
- Her makine için koordinat, gerçek ürün/sn, çevrim süresi, gerçek/azami kW ve duruş sebebi listelenir.
- Her santral için gerçek/azami kW, yakıt stoku ve tüketim/sn gösterilir.
- P0 `spatial-sim.js` kanıt çekirdeği pakette korunur ve 20/20 assertion geçer; henüz ana UI/tick/save akışına bağlı değildir.

Teknik ayrıntılar: `docs/U4_3_3_FACTORY_INTELLIGENCE.md`  
...
Node/regresyon zinciri (**27 paket**: 26 oyun/altyapı paketi + P0 uzaysal çekirdeğin 20 assertion testi):
...
python tests/u4-3-1-browser-smoke.py
python tests/u4-3-2-browser-smoke.py
python tests/u4-3-3-browser-smoke.py
```
...
- `reports/U4_3_3_INTEGRATION_REPORT.md`
- `reports/P0_SPATIAL_SIM_REPORT.md`
