...
- İnşa paleti gereksinimleri simge + ürün adı + adet biçimine geçirildi; ücretsiz haklar ve teknik metadata metinle ayrıştırıldı.
- Yerel tehdit, ilk savunma araştırması `defenseGrid` tamamlanana kadar tamamen kapatıldı.
- Savunma araştırması tamamlanınca yerel tehdit zamanlayıcısı ve tek seferlik istihbarat raporu oluşturulur.
- HUD ve Galaksi ekranı kilitli/aktif Yeryüzü Cephesi durumunu açık biçimde gösterir.
- Fabrika haritasına açılır anlık üretim ve güç durum çekmecesi eklendi.
- Makine bazında koordinat, ürün/sn, çevrim süresi, gerçek/azami kW ve duruş sebebi raporlanır.
- Santral bazında gerçek/azami kW, yakıt miktarı ve tüketim/sn raporlanır.
- P0 uzaysal simülasyon kanıtı (`spatial-sim.js`) pakete korunarak alındı; 20/20 assertion PASS, canlı entegrasyon henüz feature-flag köprüsünü bekler.
- Yeni Node ve Chromium kabul testleri; gereksinim metinlerini, savunma kapısını, canlı çekmeceyi, ARIA ve mobil taşmayı doğrular.
