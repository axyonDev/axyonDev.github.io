...
function totalFilled(g){let n=0;for(let r=0;r<ROWS;r++)for(let c=0;c<COLS;c++)if(g[r][c])n++;return n;}
function nearFullCols(g, thresh){let n=0;for(let c=0;c<COLS;c++){let h=0;for(let r=0;r<ROWS;r++)if(g[r][c])h++;if(h>=thresh)n++;}return n;}

// X2: Sonraki taş büyükse VE eşleşme ihtimali varsa kullan
function shouldX2(g, nextVal, gems, goalVal) {
  if (gems < POWER_COST.x2 || nextVal < CONFIG.power.x2MinVal) return false;
  // Gridde aynı veya 2 katı varsa kesin kullan
  for (let r=0;r<ROWS;r++) for (let c=0;c<COLS;c++)
    if (g[r][c] === nextVal || g[r][c] === nextVal*2) return true;
  // Hedefe yaklaşıyorsak (nextVal*2 >= goalVal/2) da kullan
  if (nextVal * 2 >= goalVal / 2) return true;
  return false;
}

// SMASH: Tehlikeli kolondaki işe yaramaz bloğu sil
// Daha düşük eşik: 6+ yükseklik (eski: 7), daha büyük max değer: 32 (eski: 16)
function smashTarget(g, gems) {
  if (gems < POWER_COST.smash) return null;
  let worst = null, worstScore = Infinity;
  for (let c=0;c<COLS;c++) {
    const h = colHeight(g, c);
    if (h < 6) continue; // eski: smashMinHeight=7, şimdi daha erken tepki ver
    const tr = topRow(g, c); if (tr === -1) continue;
    const v = g[tr][c];
    if (v > 32) continue; // eski: smashMaxVal=16, biraz daha büyükleri de smash et
    // Bu taş merge olabilir mi?
    let canMerge = false;
    for (let dc=-1;dc<=1;dc++) {
      const nc=c+dc; if(nc<0||nc>=COLS) continue;
      for (let dr=-1;dr<=1;dr++) {
        const nr=tr+dr; if(nr<0||nr>=ROWS) continue;
        if (g[nr][nc]===v && !(dr===0&&dc===0)) { canMerge=true; break; }
      }
      if (canMerge) break;
    }
    if (canMerge) continue; // Merge olabiliyorsa smash etme
    const score = v - h * 15; // yüksek kolon + küçük değer = öncelikli
    if (score < worstScore) { worstScore = score; worst = {r:tr, c}; }
  }
  return worst;
}

// SWAP: Aynı değerli blokları birleştirmek için daha geniş arama
function swapTarget(g, gems, goalVal) {
  if (gems < POWER_COST.swap) return null;
  const minV = Math.max(CONFIG.power.swapMinVal, goalVal / 16); // eski: goalVal/8 → daha düşük eşik
  // Önce yüksek değerliler için dene
  for (let targetV = goalVal; targetV >= minV; targetV /= 2) {
    // Yatay: aynı değer 1 boşlukla
    for (let r=0;r<ROWS;r++)
      for (let c=0;c<COLS-2;c++)
        if (g[r][c]===targetV && g[r][c+2]===targetV && g[r][c+1] && g[r][c+1]!==targetV)
          return {r1:r, c1:c+1, r2:r, c2:c+2};
    // Dikey: aynı değer 1 boşlukla
    for (let c=0;c<COLS;c++)
      for (let r=0;r<ROWS-2;r++)
        if (g[r][c]===targetV && g[r+2][c]===targetV && g[r+1][c] && g[r+1][c]!==targetV)
          return {r1:r+1, c1:c, r2:r+2, c2:c};
  }
  // Genel: herhangi iki büyük taşı yan yana getir
  for (let r=0;r<ROWS;r++)
    for (let c=0;c<COLS-2;c++)
...
// ─── TERMİNAL ────────────────────────────────────────────────────────────────
// Canlı grid paneli: ANSI cursor ile aynı satırlara yazılır (scroll olmaz).
// Milestone ve özet tablolar ise kalıcı olarak yukarı birikir.

const GRID_LINES = 8 + ROWS + 2; // panel yüksekliği (satır sayısı)
let gridPanelDrawn = false;       // ilk kez mi çiziliyor?
...
function print(g, state, moveNum, col, isHard, power) {
  ...
  // İlk çizimde boş satır rezerve et, sonraki çizimlerde cursor geri git
  if (!gridPanelDrawn) {
    for (let i = 0; i < GRID_LINES; i++) process.stdout.write('\n');
    gridPanelDrawn = true;
  }

  // Cursor'ı panel başına taşı (milestone satırları + grid paneli yukarısı)
  process.stdout.write(`\x1b[${GRID_LINES + milestoneLineCount}A`);

  const lines = [];
  lines.push(`${K.cy}╔══════════════════════════════════════════╗${K.r}`);
  lines.push(`${K.cy}║${K.b}  🤖 Crystal Drop Bot   Hamle: ${String(moveNum).padEnd(6)}${K.cy}  ║${K.r}`);
  lines.push(`${K.cy}╠══════════════════════════════════════════╣${K.r}`);
  lines.push(`${K.cy}║${K.r}  Skor: ${K.y}${String(state.score).padEnd(9)}${K.r} 💎 ${K.gr}${String(state.gems).padEnd(6)}${K.r}   ${K.cy}║${K.r}`);
  ...
  lines.push(`${K.cy}║${K.r}  Sonraki:${K.b}${K.y}${String(val).padEnd(5)}${K.r} → ${K.d}${state.nextNextVal}${K.r}               ${K.cy}║${K.r}`);
  lines.push(`${K.cy}╚══════════════════════════════════════════╝${K.r}`);
  ...
  for(let r=0;r<ROWS;r++){
    ...
    for(let c=0;c<COLS;c++){
      const v=g[r][c];
      if(!v)                      row+=`${K.d}  · ${K.r}`;
      else if(v>=state.goalVal)   row+=`${K.gr}${K.b}${String(v).padStart(3)} ${K.r}`;
      else if(v>=state.goalVal/2) row+=`${K.y}${String(v).padStart(3)} ${K.r}`;
      else                        row+=`${String(v).padStart(3)} `;
    }
    lines.push(row);
  }
  ...
  let status='';
  if(power)       status=`  ${K.m}⚡ POWER: ${power.toUpperCase()}${K.r}`;
  else if(isHard) status=`  ${K.m}🤔 Zor karar...${K.r}           `;
  else            status=`  ▼ Kolon ${col} → ${val}                `;
  lines.push(status);

  // Her satırı yaz + satır sonunu temizle (eski karakter kalmasın)
...
(async()=>{
  ...
  function checkMilestone(goalVal, score) {
    if (lastGoalVal !== -1 && goalVal !== lastGoalVal) {
      ...
      // Cursor'ı grid panelinin altına + önceki milestone satırlarının sonuna taşı
      process.stdout.write(`\x1b[${GRID_LINES + milestoneLineCount}B\x1b[1G`);
      process.stdout.write(`${K.m}${'─'.repeat(48)}${K.r}\n`);
      process.stdout.write(`${K.gr}${K.b}  🏆 HEDEF TAMAM: ${String(lastGoalVal).padEnd(8)}${K.r}\n`);
      ...
      // Cursor'ı grid paneli başına geri taşı
  ...
  function printFinalReport() {
    ...
    // Grid paneli + birikmiş milestone satırlarının altına tamamen geç, raporu kalıcı yaz
    process.stdout.write(`\x1b[${GRID_LINES + milestoneLineCount}B\x1b[1G`);
  ...
  while(true){
    ...
    if(powerCD===0){
      const fc = fullCols(g);
      const nfc = nearFullCols(g, ROWS - 1); // 1 hücre kalan kolonlar
      const filled = totalFilled(g);
      const fillPct = filled / (ROWS * COLS);

      // BOMB — 3+ kolon tam dolu VEYA doluluk %80+ ve 2+ kolon 7/8 dolu
      if(!usedPower && gems>=POWER_COST.bomb && (fc>=3 || (fillPct>=0.80 && nfc>=2))){
        console.log('\n💣 BOMB!'); await wait(700);
        await clickPower(page,'bomb'); usedPower='bomb'; powerCD=5; await wait(600);
      }
      // UNDO — 1+ kolon tam dolu (eski: 2)
      if(!usedPower && gems>=POWER_COST.undo && historyLen>0 && fc>=1){
        console.log('\n↩️ UNDO'); await wait(500);
        await clickPower(page,'undo'); usedPower='undo'; powerCD=3; await wait(500);
      }
      // SMASH — tehlikeli kolon + işe yaramaz blok (daha erken tetiklenir)
      if(!usedPower){
        const sm=smashTarget(g,gems);
        if(sm){
          console.log(`\n💥 SMASH (${sm.r},${sm.c}) val=${g[sm.r][sm.c]}`);
          await wait(600);
          await clickPower(page,'smash'); await wait(350);
          await clickBlock(page,sm.r,sm.c); usedPower='smash'; powerCD=3; await wait(600);
        }
      }
      // SWAP — merge fırsatı var (daha geniş arama)
      if(!usedPower){
        const sw=swapTarget(g,gems,goalVal);
        if(sw){
          console.log(`\n🔄 SWAP (${sw.r1},${sw.c1})↔(${sw.r2},${sw.c2})`); await wait(600);
      ...
      // X2 — büyük taş ve eşleşme ihtimali yüksek (daha agresif)
      if(!usedPower && !st.nextX2 && shouldX2(g,st.nextVal,gems,goalVal)){
        console.log(`\n⚡ X2 nextVal=${st.nextVal}→${st.nextVal*2}`);
    ...
    await wait(humanWait(isHard&&!usedPower));
