...
function print(g, state, moveNum, move, power) {
  ...
  console.log(`${K.cy}║${K.b}  🤖 Crystal Merge Bot   Hamle: ${String(moveNum).padEnd(5)}${K.cy}  ║${K.r}`);
  console.log(`${K.cy}╠══════════════════════════════════════════╣${K.r}`);
  console.log(`${K.cy}║${K.r}  Skor: ${K.y}${String(state.score).padEnd(9)}${K.r} 🪙 ${K.gr}${String(state.coins).padEnd(6)}${K.r} ${K.cy}║${K.r}`);
  ...
  console.log(`${K.cy}║${K.r}  Doluluk: ${fr>=90?K.re:fr>=75?K.y:K.gr}%${fr}${K.r}                       ${K.cy}║${K.r}`);
  console.log(`${K.cy}╚══════════════════════════════════════════╝${K.r}`);
  ...
  for (let r = 0; r < ROWS; r++) {
    ...
    for (let c = 0; c < COLS; c++) {
      ...
      else if (isCenter)    process.stdout.write(`${K.cy}${K.b}${String(v).padStart(4)} ${K.r}`);
      else if (inMove)      process.stdout.write(`${K.gr}${String(v).padStart(4)} ${K.r}`);
      else if (v>=state.goalVal) process.stdout.write(`${K.gr}${K.b}${String(v).padStart(4)} ${K.r}`);
      else if (v>=state.goalVal/4) process.stdout.write(`${K.y}${String(v).padStart(4)} ${K.r}`);
      else                  process.stdout.write(`${String(v).padStart(4)} `);
    }
    process.stdout.write('\n');
  }
  ...
  if (power) {
    ...
    console.log(`\n  ▼ (${move.r},${move.c}) → ${move.n} taş merge → ${move.newV} [skor:+${Math.round(move.score)}]`);
  } else {
    console.log(`\n  ${K.re}⚠ Hamle bulunamadı!${K.r}`);
  }
}
  ...
  function checkMilestone(st) {
    if (st.targetIdx > lastTargetIdx) {
      ...
      for (let i = lastTargetIdx + 1; i <= st.targetIdx; i++) {
        ...
        // Anlık kutlama çıktısı
        const K = { r:'\x1b[0m', cy:'\x1b[36m', y:'\x1b[33m', gr:'\x1b[32m', b:'\x1b[1m', m:'\x1b[35m' };
        console.log(`\n${K.m}${'─'.repeat(48)}${K.r}`);
        console.log(`${K.gr}${K.b}  🏆 HEDEF TAMAM: ${fmtVal(target).padEnd(8)}${K.r}`);
